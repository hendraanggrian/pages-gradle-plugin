...
internal class MinimalWriter(val options: MinimalPageOptionsImpl) {
    companion object {
        ...
        get() {
            ...
            return createHTMLDocument().html {
                head {
                    ...
                    title(projectTitle)
                    options.favicon?.let { link(rel = "icon", href = it) }
                    ...
                    div(classes = "wrapper") {
                        header {
                            ...
                            if (options.headerButtons.isNotEmpty()) {
                                ul {
                                    options.headerButtons.forEach { (text, url) ->
                                        val line1 = text.substringBefore('\n')
                                        val line2 = text.substringAfter('\n')
    ...
    val mainCss: String
        get() = """
            ...
            header li {
              width: ${if (options.headerButtons.size == 0) HEADER_WIDTH else HEADER_WIDTH / options.headerButtons.size - 1}px;
              float: left;
            ...
            @media print, screen and (max-width: 480px) {
              ...
              header li, header ul li + li + li {
                width: ${if (options.headerButtons.size == 0) HEADER_WRAPPED_WIDTH_PERCENTAGE else HEADER_WRAPPED_WIDTH_PERCENTAGE / options.headerButtons.size}%;
              }
